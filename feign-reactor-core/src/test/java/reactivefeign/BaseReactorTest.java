...
abstract public class BaseReactorTest {
    ...
    public static void installBlockHound() {
        if (INSTALL_BLOCKHOUND) {
            try {
                ...
                builder.install();
                blockHoundInstalled = verifyBlockHoundFunctional();
                if (!blockHoundInstalled) {
                    System.err.println("BlockHound installed but not functional on this JDK");
                }
            } catch (Throwable t) {
                System.err.println("BlockHound installation failed (likely unsupported JDK): " + t.getMessage());
            }
        }
    }

    private static boolean verifyBlockHoundFunctional() {
        try {
    ...
    public void shouldFailAsBlocking() {
      assumeTrue(blockHoundInstalled, "BlockHound not available on this JDK");
